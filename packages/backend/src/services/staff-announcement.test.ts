...
import { AddStaffAnnouncement, DeleteStaffAnnouncement, GetJuriesFromStaffAnnouncement, GetStaffAnnouncementById, GetStaffAnnouncements, GetStaffAnnouncementsByJuryIdNumber, UpdateDisciplineRulesAtStaffAnnouncement, UpdateStaffAnnouncement } from "./staff-announcement";
...
describe("Staff Announcement Service Test", () => {
    ...
    it("Get Staff Announcements By Jury", async () => {
        ...
        const staffAnnouncements = await GetStaffAnnouncementsByJuryIdNumber(jury?.id_number!);
        expect(staffAnnouncements).toBeDefined();
        expect(staffAnnouncements.length).toBeGreaterThan(0);
    });
