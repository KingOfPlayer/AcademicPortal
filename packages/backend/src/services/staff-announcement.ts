...
export const AddStaffAnnouncement = async (
  staffAnnouncement: StaffAnnouncementDTO,
) => {
  const announcement = new StaffAnnouncement(staffAnnouncement);
  await announcement.save();
};

export const GetStaffAnnouncements = async (): Promise<
  StaffAnnouncementDTO[]
> => {
  return await StaffAnnouncement.find({}).sort({ createdAt: -1 });
};

export const GetStaffAnnouncementById = async (
  id: string,
): Promise<StaffAnnouncementDTO | null> => {
  return await StaffAnnouncement.findById(id);
};

export const UpdateStaffAnnouncement = async (
  id: string,
  staffAnnouncement: StaffAnnouncementDTO,
): Promise<StaffAnnouncementDTO | null> => {
  return await StaffAnnouncement.findByIdAndUpdate(id, staffAnnouncement, {
    new: true,
  });
};

export const DeleteStaffAnnouncement = async (
  id: string,
): Promise<StaffAnnouncementDTO | null> => {
  return await StaffAnnouncement.findByIdAndDelete(id);
};

export const GetJuriesFromStaffAnnouncement = async (
  id: string,
): Promise<UserDTO[]> => {
  const staffAnnouncement = await StaffAnnouncement.findById(id).populate<{
    juries: IUser[];
  }>("juries");
...
export const GetStaffAnnouncementsByJuryIdNumber = async (
  id_number: number,
...
export const UpdateDisciplineRulesAtStaffAnnouncement = async (
  id: string,
): Promise<StaffAnnouncementDTO | null> => {
  ...
  return await StaffAnnouncement.findByIdAndUpdate(
    id,
