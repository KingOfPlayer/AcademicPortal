...
export default async function (fastify: FastifyInstance & FastifyAuthPlugin) {
  ...
  fastify.put<{ Body: IStaffAnnouncement }>(
    "/:id",
    {
      preHandler: [
    ...
    async (request, reply) => {
      ...
      const updatedStaffAnnouncement = await UpdateStaffAnnouncement(
        id,
        request.body,
      );
  ...
  fastify.delete(
    ...
    async (request, reply) => {
      ...
      return reply
        .status(200)
        .send({ message: "Staff announcement deleted successfully" });
    },
  );

  fastify.put(
    "/update/discipline-rules/:id",
    {
      preHandler: [
        fastify.authentication([UserRoles.Admin, UserRoles.Administrator]),
      ],
    ...
    async (request, reply) => {
      ...
      const updatedStaffAnnouncement =
        await UpdateDisciplineRulesAtStaffAnnouncement(id);
      return reply.status(200).send(updatedStaffAnnouncement);
    },
  );
}
